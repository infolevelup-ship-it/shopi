...
export function QuoteForm({
  ...
  useEffect(() => {
    if (!preselectedCustomerId || initial?.customer) return;
    let cancelled = false;
  ...
  }, [preselectedCustomerId, initial?.customer]);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result =
        isEdit && quoteId
          ? await updateQuoteAction({ quoteId, ...common })
          : await createQuoteAction({ customerId: customer.id, ...common });
      if (!result.ok) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader
        back={
          isEdit && quoteId
            ? { href: `/quotes/${quoteId}`, label: quoteNumber ?? "Cotización" }
            : { href: "/quotes", label: "Cotizaciones" }
        }
        title={isEdit ? `Editar ${quoteNumber ?? "cotización"}` : "Nueva cotización"}
      />
        ...
        <SCard title="Cliente">
          {customer ? (
            <div className="flex items-center justify-between gap-3 rounded-xl border border-line bg-surface-soft px-3 py-3">
              ...
              {/* Al editar no se cambia el cliente: una cotización para otro
                  cliente es otra cotización, no una edición de esta. */}
              {!isEdit && (
                <button
                  type="button"
